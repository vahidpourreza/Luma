...
public class QueryDispatcherDomainExceptionHandlerDecorator : QueryDispatcherDecorator
{
    ...
    public override async Task<QueryResult<TData>> Execute<TQuery, TData>(TQuery query)
    {
        ...
        {
            _logger.LogError(LumaEventId.DomainValidationException, ex, "Processing of {QueryType} With value {Query} failed at {StartDateTime} because there are domain exceptions.", query.GetType(), query, DateTime.Now);
