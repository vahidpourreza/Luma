...
public class EventDispatcherValidationDecorator : EventDispatcherDecorator
{
    ...
    {
        _logger.LogDebug(LumaEventId.EventValidation, "Validating Event of type {EventType} With value {Event}  start at :{StartDateTime}", @event.GetType(), @event, DateTime.Now);

        List<string> errorMessages = Validate(@event);

        if (errorMessages.Any())
        {
            _logger.LogInformation(LumaEventId.EventValidation, "Validating query of type {QueryType} With value {Query}  failed. Validation errors are: {ValidationErrors}", @event.GetType(), @event, errorMessages);
        }
        else
        {
            _logger.LogDebug(LumaEventId.EventValidation, "Validating query of type {QueryType} With value {Query}  finished at :{EndDateTime}", @event.GetType(), @event, DateTime.Now);
    ...
    private List<string> Validate<TDomainEvent>(TDomainEvent @event)
    {
        ...
        {
            _logger.LogInformation(LumaEventId.CommandValidation, "There is not any validator for {EventType}", @event.GetType());
        }
