...
public class CommandDispatcherDomainExceptionHandlerDecorator : CommandDispatcherDecorator
{
    ...
    public override async Task<CommandResult> Send<TCommand>(TCommand command)
    {
        ...
        {
            _logger.LogError(LumaEventId.DomainValidationException, ex, "Processing of {CommandType} With value {Command} failed at {StartDateTime} because there are domain exceptions.", command.GetType(), command, DateTime.Now);
        ...
        catch (AggregateException ex)
        {
            ...
            {
                _logger.LogError(LumaEventId.DomainValidationException, domainStateException, "Processing of {CommandType} With value {Command} failed at {StartDateTime} because there are domain exceptions.", command.GetType(), command, DateTime.Now);
    ...
    private string GetExceptionText(DomainStateException domainStateException)
    {
        ...
        _logger.LogInformation(LumaEventId.DomainValidationException, "Domain Exception message is {DomainExceptionMessage}", result);
